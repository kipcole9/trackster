  ...
  task(:build_tracker => :environment) do
    ...
    begin
      if Rails.env == "staging" || Rails.env == "development"
        run "sed -e 's/{{SITE}}/#{Trackster::Config.site}:8080/' #{tracker_template} > #{tracker_debug}"
      elsif Rails.env = "production"
        run "sed -e 's/{{SITE}}/#{Trackster::Config.site}/;s/console.log/\/\/ console.log/' #{tracker_template} > #{tracker_production}"
      end
    rescue Exception => e
