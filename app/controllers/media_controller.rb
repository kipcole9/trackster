class MediaController < ApplicationController
  ...
  def maxplay
    ...
    view_data = Track.video_playtime.label(options[:label]).between(options[:period]).all
...
private
  ...
  def views_grouped_by_value(view_data)
    playtimes = []    
    view_data.group_by(&:value).each do |playtime, views|
  ...
  def round_to_nearest!(data, rounding = 5)
    data.each {|d| (d.value = ((d.value  / rounding).to_i * rounding))}
  end
