panel t('panels.new_user'), :display_errors => :user  do
  block do
    form_for @user || User.new do |user|
      fieldset t('users.new_user') do
        user.text_field :email  # Don't validate live since it leaks email addresses if they exist,       :validate => :validations
